...
var enableAddition = false, additionMade = false;
...
function hierarchyLayout() {
  ...
  var link_elements = zoomableField.append("g").attr("class", "links").selectAll(".link"),
      node_elements = zoomableField.append("g").attr("class", "nodes").selectAll(".node");
  ...
  function update() {
    ...
    link_elements = link_elements.data(links, function(d) { return d.id; });
    link_elements.exit().remove();

    node_elements = node_elements.data(nodes, function(d) { return d.name; });
    node_elements.exit().remove();

    var nodeEnter = node_elements.enter().append("g")
      .attr("class", "node")
      .on("click", click)
      .call(drag);
    ...
    nodeEnter.append("circle")
      .attr("r", size)
      .attr("stroke-width", 1.0 / currentZoomLevel + "px");
  ...
  function updateLinks(error, addLinks) {
    ...
    link_elements.enter().insert("line", ".node")
      .attr("class", "link")
  ...
  function zoomed() {
    ...
    zoomableField.attr("transform", "translate(" + d3.event.translate + ")scale(" + currentZoomLevel + ")");
    node_elements.select("circle")
      .attr("r", size)
      .attr("stroke-width", 1.0 / currentZoomLevel + "px");
    node_elements.select("text")
      .attr("font-size", 1.0 / currentZoomLevel + "em");
  ...
  d3.select("body").on('keydown', function () {
    ...
    case 65:
    case 37:
        zoom.translate([zoom.translate()[0] + step, zoom.translate()[1]]);
        break;
    case 68:
    case 39:
        zoom.translate([zoom.translate()[0] - step, zoom.translate()[1]]);
    ...
    case 87:
    case 38:
        zoom.translate([zoom.translate()[0], zoom.translate()[1] + step]);
        break;
    case 83:
    case 40:
        zoom.translate([zoom.translate()[0], zoom.translate()[1] - step]);
    ...
    zoomableField.attr("transform", "translate(" + zoom.translate() + ")scale(" + currentZoomLevel + ")");
  });
  
  function colorChildren(node) {
    var count = node.children.length, current = 0;
    scale = chroma.scale(["red","orange","yellow","green","blue","indigo","violet"]).mode('lab');
  ...
  function size(d) {
    if(d.size) return Math.max( Math.sqrt(d.size) / 5 , 5.0 ) / currentZoomLevel;
    else return 5.0 / currentZoomLevel;
  }
  ...
  function expand(n) {
    if (n.children) {
      ...
      if(isNaN(n.name)) alert("Leaf id: " + n.name);
      else window.open("http://oeis.org/A" + n.name);
    }
  }
  ...
  function flatten(root) {
    ...
    function recurse(node) {
      ...
      else if (node.children) node.children.forEach(recurse);
    }
...
function relationLayout() {
  ...
  var svg = d3.select("body").append("svg");
  var largestField = svg.append("g").attr("class", "fullField");
  largestField.append("rect").call(zoom);
  var zoomableField = largestField.append("g").attr("class", "zoomableField");
  var link_elements = zoomableField.append("g").attr("class", "links").selectAll(".link"),
      node_elements = zoomableField.append("g").attr("class", "nodes").selectAll(".node");
  ...
  function click(d) {
    if (d3.event.defaultPrevented) return; // ignore drag
    if(isNaN(d.name)) alert("ID: " + d.name);
    else window.open("http://oeis.org/A" + d.name);
  }
...
function addToRelationView() {
  if(enableAddition) {
    ...
    if(primaryNodes.length == 0) queryString = queryString.concat("NONE");
    else {
      first = true;
      for(i = 0; i < primaryNodes.length; i++) {
        ...
        queryString = queryString.concat(primaryNodes[i].name);
      }
    }
    console.log(queryString);
      ...
      if(json.nodes) {
        ...
        while(nodes.length > 0 && json.nodes.length > 0) {
          if(nodes[0].name == json.nodes[0].name) {
            ...
            newLinks.push(json.nodes.shift());
          }
        }
        if(nodes.length == 0) {
